...
router.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'aset-tronics-api', time: new Date().toISOString() });
});
...
router.get('/dashboard', (req, res) => {
  ...
  const totalTime = round(operations.reduce((s, o) => s + Number(o.time || 0), 0), 2);
  res.json({
    ...seed.dashboard,
...
router.get('/projects/:id', (req, res) => {
  ...
  const operations = store.getAll('operations').filter((o) => String(o.projectId) === String(p.id));
...
router.get('/operations', (req, res) => {
  ...
  if (req.query.projectId) ops = ops.filter((o) => String(o.projectId) === String(req.query.projectId));
  const totalTime = round(ops.reduce((s, o) => s + Number(o.time || 0), 0), 2);
...
router.get('/tools', (req, res) => {
  ...
  if (q) tools = tools.filter((t) => (t.code + ' ' + t.name + ' ' + t.type).toLowerCase().includes(String(q).toLowerCase()));
  if (type) tools = tools.filter((t) => t.type.toLowerCase() === String(type).toLowerCase());
  if (recommended === 'true') tools = tools.filter((t) => t.recommended);
  res.json(tools);
});
router.post('/tools', (req, res) => res.status(201).json(store.insert('tools', req.body || {})));
...
router.get('/calculator/materials', (req, res) => {
  res.json(Object.keys(machining.MATERIALS).map((name) => ({ name, ...machining.MATERIALS[name] })));
});
router.post('/calculator/milling', (req, res) => res.json(machining.milling(req.body || {})));
router.post('/calculator/turning', (req, res) => res.json(machining.turning(req.body || {})));
router.post('/calculator/drilling', (req, res) => res.json(machining.drilling(req.body || {})));
